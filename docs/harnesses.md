...
a classification that asserts nothing for it to check, and `metrics` refuses a
`src` too small to be this repository's. Without the split, an empty `src` would
score a perfect card: no files means no cycles, no unused exports, and no escape
...
per failing row in the `quarantine.json` format, exact in both directions. It is
EMPTY today, so the gate is a pure regression gate: all 700 rows render the same
after formatting, and a row that starts diverging fails until it is fixed or its
entry is written. There is no triage script and deliberately so: entries over a
fixed 700-row product are regenerated by hand in one command, and a script for
it is scaffolding the family does not need.

### The spelling reduction order

`tests/conformance/reduction-order.ts` weighs a document on the five-count
lexicographic order that every conversion must strictly decrease
(docs/architecture.md, "The spelling reduction order"). Its two obligations are
gated in two places because they have two domains, and the module's own comment
is where the derivation, the domain, the two limits and the coordinates the
check does not reach are written down; this entry says only which gate runs
where.

NON-INCREASE is the `reduction` differential property, run on all 1,614 corpus
cases: a format that raised the order applied a rule some other rule could undo,
which is the loop termination forbids. STRICT DECREASE is
`tests/conformance/reduction-order.test.ts`, over the confluence roster
(`confluence-variants.ts`): every row the formatter converges is a landed
conversion whose losing spelling must descend the order, and the rows the order
does not separate are pinned exactly. The same file carries the perturbation
proof, one rewrite per component built in the test that runs a landed conversion
backwards, and the constructed pair that pins the components' sequence, so the
check cannot pass by never being tested. The registry sweep is deliberately not
one of its domains.

### `bun run triage` — the conformance sweep

Assesses every corpus case against the five differential properties (crash,
idempotency, fidelity, reading, and `reduction`, the reduction-order
non-increase check from issue #220) and groups the failures by signature.
`--write` regenerates `tests/conformance/quarantine.json`: still-failing cases
keep their issue tag, new failures are tagged `UNTRIAGED`, and cases that now
pass are dropped — which is how a fix gets pinned, because the manifest asserts
exact agreement and the suite fails if a quarantined case starts passing.
...
grids, `metrics --base`, and `deletion-gate --base`, serially in one job,
