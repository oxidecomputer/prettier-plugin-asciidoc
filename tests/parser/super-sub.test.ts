 ...
 * `@asciidoctor/core` actually does. Which span survives where two of
 * them overlap is a different question, asked at the bottom of this
...
interface Row {
  ...
  /**
   * The formatted bytes, when they are not the source's own. Almost
   * every row here IS a fixed point - neither of these two rows has a
   * respelling for the printer to choose - and the one exception is a
   * pair the source spelled across two lines, which no pair can hold,
   * so reflow joins the lines as it would for any other prose.
   */
 ...
 *
 * The byte expectation defaults to the source, because these two rows
 * have no respelling for the printer to choose; a row that reflow
 * legitimately moves says so in its own `formatted` field.
...
describe("the superscript and subscript rows", () => {
  ...
  const ROWS: readonly Row[] = [
    ...
    },
    {
      ...
      source: "x ^a^b^ y",
      oracleContains: "x <sup>a</sup>b^ y",
      delimiters: [2, 4],
    ...
    },
    {
      ...
      source: "x ^a^^b^ y",
      oracleContains: "x <sup>a</sup><sup>b</sup> y",
      delimiters: [2, 4, 5, 7],
    ...
    },
    {
      ...
      source: "x ~a~b~ y",
      oracleContains: "x <sub>a</sub>b~ y",
      delimiters: [2, 4],
  ...
  const ROWS: readonly Row[] = [
    {
      ...
      source: "x [a^b]^c^ y",
      oracleContains: '<sup class="a^b">c</sup>',
      delimiters: [7, 9],
    ...
    },
    {
      ...
      source: "x [red]^a^ y",
      oracleContains: '<sup class="red">a</sup>',
      delimiters: [7, 9],
    ...
    },
    {
      ...
      oracleContains: String.raw`x \[]<sup>a</sup> y`,
      delimiters: [5, 7],
  ...
  const ROWS: readonly Row[] = [
    {
      ...
      source: "x ^*a*^ y",
      oracleContains: "x <sup><strong>a</strong></sup> y",
      delimiters: [2, 6],
    ...
    },
    {
      ...
      source: "x ^#a#^ y",
      oracleContains: "x <sup><mark>a</mark></sup> y",
      delimiters: [2, 6],
    ...
    },
    {
      ...
      source: "x *^a^* y",
      oracleContains: "x <strong><sup>a</sup></strong> y",
      delimiters: [3, 5],
    ...
    },
    {
      ...
      source: "x ^*a b*^ y",
      oracleContains: "x ^<strong>a b</strong>^ y",
      delimiters: [],
      shape: ['"x ^"', 'boldc["a b"]', '"^ y"'],
    },
    {
      ...
      source: "x ^a~b~c^ y",
      oracleContains: "x <sup>a<sub>b</sub>c</sup> y",
      delimiters: [2, 4, 6, 8],
...
describe("where no attrlist is taken, the scan and the oracle agree", () => {
  ...
  const ROWS: readonly DivergenceRow[] = [
    {
      source: "x ^[red]c^ y",
      oracleContains: "x <sup>[red]c</sup> y",
      shape: ['"x "', 'superscript["[red]c"]', '" y"'],
    },
    {
      source: "x ^[]#c#^ y",
      oracleContains: "x <sup>[]<mark>c</mark></sup> y",
    ...
    {
      source: "x ^[red]#c#a^ y",
      oracleContains: "x <sup>[red]#c#a</sup> y",
