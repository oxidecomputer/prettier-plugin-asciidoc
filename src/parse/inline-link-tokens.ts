 ...
 * immediately before a newline forces a line break in output. Uses a
 * lookahead for `\n` rather than consuming it — the newline is left
 * for InlineNewline. Must appear before InlineNewline in the inline
 * mode token list.
 *
 * Context-free on purpose. The one shape Asciidoctor reads as a
 * literal `+` instead — a whitespace-only ` +` line directly under a
 * list item's marker line whose indent `adjust_indentation!` strips —
 * is a property of the whole list-item paragraph, and the BlockReader
 * decides it where it reads that paragraph (see the literal-plus rule
 * in src/parse/lines/paragraph-reader.ts); the inline lexer never
 * looks past the fragment it is given.
 */
