 ...
 * it, or put it alone on the author's line. Both answers write the
 * same characters in the same order, so the choice is about where the
 * packer may BREAK and never about bytes.
 */
 ...
 * constrained formatting. `"detached"` keeps the mark as an atom of
 * its own with the source's break behind it, which is the only legal
 * placement beside a raw line.
 */
type OpenMarkPlacement = "fused" | "detached";
...
export function markPlacement(
  ...
  return {
    openPlacement: openMarkPlacement(inner),
    detachClose:
 ...
 */
function openMarkPlacement(inner: readonly Atom[]): OpenMarkPlacement {
  return inner[0].ownsItsLine ? "detached" : "fused";
}
...
export function pushSpanAtoms(
  ...
  if (openPlacement === "fused") {
    ...
    // `"detached"` writes the author's break, where no other
    // placement is legal.
    out.push(
 ...
 * whitespace they stood for. Split from `appendSpan` (inline.ts) for
